...
翻译:关联列表Emacs Lisp库提供了一个全面的API来处理alist。它是由Troy Pracy编写的，最新版本是0.6.1。这是GNU Emacs系列的第八篇文章，探讨了用于创建、检索和更新alist的正则表达式和反身函数变体。还回顾了映射、过滤、折叠和循环alist的结构。
...
翻译:这篇文章中展示的源代码可以在https://github.com/troyp/asoc.el找到，并且已经发布了GNU通用公共许可证v2版本。
...
翻译:asoc.el 源文件可以从 https://github.com/troyp/asoc.el 获取，并将其添加到您的 Emacs 初始配置文件中，以便在启动时加载。
You can then use the library with the following Emacs Lisp code snippet:
翻译:你可以使用以下emacs lisp代码片段来使用这个库：
...
翻译:我们将从asoc.el中用于创建关联列表的构造函数API开始。
...
翻译:作为ocopy函数返回一个通过参数传递给它的alist的浅拷贝。例如：
...
翻译:您可以使用asoc-zip API将键和值组合起来创建一个关联列表。如果键的数量多于值的数量，那么这些多余的键将具有nil值。下面给出了几个示例：
...
翻译:如果你想要返回一个去除了重复键的alist，你可以使用asoc-uniq函数。在下面的例子中，只返回了键b的第一个值。
...
翻译:asoc-merge 函数有助于合并多个关联列表（alist）。如果在多个列表中出现相同的键，则后者关联列表中的键优先。如果同一关联列表中存在相同的关键字，则第一个出现的位置具有优先权。下面展示了几个示例：
...
翻译:你可以使用`asoc-sort-keys`函数来根据键对元素进行排序并返回一个列表。例如：
...
翻译:作为过滤器函数，asoc-filter接受一个谓词函数和一个关联列表（alist）。它返回一个新的关联列表，其中只包含那些满足谓词函数条件的元素。在下面的例子中，所有值小于3的元素的值都被返回了。
...
翻译:关联过滤器函数的指代变体是asoc--filter。它返回形式为真的alist元素。下面给出一个使用asoc--filter编写的示例：
